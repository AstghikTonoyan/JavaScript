...
const app = {
    ...
    isOpen (time){
        if (time >= 9){
            ...
            app.restaurantDetails[6].isDelivery = undefined; 
    ...
    staffPlane(n){
        ...
        if(n <= 15){
            ...
            app.restaurantStaff[13].customerHelper;
        } else if(n >= 15){
    ...
    callCenter(address, telNumber, isDelivery, isOpen, type) {
        return `
            Ողջույն՜, Ձեզ սպասարկում է ${app.restaurantStaff[13].customerHelper}ը
            ${
    ...
    cookerShift (n){ 
        if(n>=15){ if(app.restaurantDetails[7].isOpen === false) {
            "";
        } else if (n>15){
        ...
        if(n<=15){ if(app.restaurantDetails[7].isOpen === false) {
            ...
            console.log("Հարգելի հաճախորդ, կներեք այսօր "+app.restaurantStaff[9].cook + "ի հանգստյան օրն է");
        } }
    },   
...
const client = app.callCenter(10);
console.log(client);
